...
// Differentiates personal expenses (Food, Auto, Fuel, Medicine, Self, etc.)
// from Real People (Adarsh, Sagar, Ritesh, etc.) so AI never treats self-expenses as money to collect!

const PERSONAL_CATEGORY_KEYWORDS = [
 'self', 'ghar', 'home', 'family', 'food', 'auto', 'fuel', 'petrol', 'diesel',
 'medicine', 'doctor', 'hospital', 'medical', 'health', 'clinic', 'pharmacy',
 'office', 'bank charges', 'bank charge',
 'bank fee', 'shopping', 'groceries', 'grocery', 'kirana', 'supermarket', 'recharge',
 'mobile recharge', 'phone recharge', 'wifi', 'internet', 'broadband', 'electricity',
 'electricity bill', 'bijli bill', 'power bill', 'rent', 'house rent', 'room rent',
 'shop rent', 'travel', 'bus', 'train', 'flight', 'ticket', 'cab', 'taxi', 'uber',
 'ola', 'rapido', 'snacks', 'tea', 'chai', 'coffee', 'hotel', 'restaurant', 'cafe',
 'swiggy', 'zomato', 'gym', 'fitness', 'fees', 'college fees', 'school fees',
 'tuition', 'coaching', 'subscription', 'netflix', 'prime', 'spotify', 'youtube',
 'entertainment', 'movie', 'cinema', 'bills', 'utilities', 'water bill', 'gas bill',
 'cylinder', 'donation', 'charity', 'mandir', 'repair', 'maintenance', 'bike service',
 'car service', 'clothes', 'clothing', 'salon', 'parlour', 'barber', 'haircut',
 'laundry', 'dry clean', 'books', 'stationery', 'cash', 'upi', 'bank', 'unknown',
...
const INVESTMENT_KEYWORDS = [
 ...
 'share market', 'trading', 'sip', 'investment', 'investments', 'other investment',
  'apj emi', 'emi', 'loan', 'car emi', 'bike emi', 'home loan'
];

const INCOME_KEYWORDS = [
 'salary', 'bank intt', 'bank interest', 'interest', 'dividend', 'other income', 'cashback', 'bonus'
];

function isInvestmentCategory(name){
 if (!name) return false;
 const s = String(name).trim().toLowerCase();
 return INVESTMENT_KEYWORDS.some(k => s === k || s.includes(k));
}

function isIncomeCategory(name){
 if (!name) return false;
 const s = String(name).trim().toLowerCase();
 return INCOME_KEYWORDS.some(k => s === k || s.includes(k));
}
...
function isPersonalCategory(name){
 ...
 return PERSONAL_CATEGORY_KEYWORDS.some(k => s === k || s.includes(k));
}
...
function openAiChatModal(){
 ...
 <div id="aiChatModal" style="position:fixed;inset:0;width:100%;height:100%;background:#06080e;z-index:2000;display:flex;flex-direction:column;font-family:'Inter',sans-serif;color:#f3f4f6;">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;padding:calc(12px + env(safe-area-inset-top,0px)) 16px 12px;border-bottom:1px solid rgba(255,255,255,0.08);background:rgba(10,12,18,0.85);backdrop-filter:blur(16px);flex-shrink:0;">
   <div style="display:flex;align-items:center;gap:10px;">
    <svg viewBox="0 0 24 24" width="28" height="28" style="filter:drop-shadow(0 0 10px rgba(155,114,203,0.8));flex-shrink:0;">
     <defs>
      <linearGradient id="chatGeminiGrad" x1="0%" y1="0%" x2="100%" y2="100%">
       <stop offset="0%" stop-color="#4E82EE" />
       <stop offset="35%" stop-color="#9B72CB" />
       <stop offset="70%" stop-color="#D96570" />
       <stop offset="100%" stop-color="#F4AF5F" />
      </linearGradient>
     </defs>
     <path fill="url(#chatGeminiGrad)" d="M12 0C12 6.627 6.627 12 0 12C6.627 12 12 17.373 12 24C12 17.373 17.373 12 24 12C17.373 12 12 6.627 12 0Z"/>
    </svg>
    <div>
     <div style="font-size:15px;font-weight:700;letter-spacing:-0.2px;background:linear-gradient(135deg,#fff,#c9c9e8);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">Gemini Live</div>
  ...
  <div id="aiChatList" style="flex:1;overflow-y:auto;padding:16px 14px;display:flex;flex-direction:column;gap:14px;"></div>

  <!-- Bottom Floating Input & Gemini Live Voice Bar -->
...
async function stopAiChatVoiceCapture(hasSpokenManual){
 ...
 if (msgEl) msgEl.innerHTML = '<span style="color:#9B72CB;">✨ Gemini is thinking & answering...</span>';
...
${summary}`;
  ...
  if (msgEl) msgEl.textContent = "⚠️ Gemini error: " + err.message;
 }
}
...
function renderAiChatMessages(nameOverride){
 ...
 if (aiChatMessages.length === 0){
  ...
  <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;margin:auto;text-align:center;padding:20px 10px;max-width:340px;">
   ...
   <div style="margin-bottom:16px;">
    <svg viewBox="0 0 24 24" width="56" height="56" style="filter:drop-shadow(0 0 22px rgba(155,114,203,0.9));">
     <path fill="url(#chatGeminiGrad)" d="M12 0C12 6.627 6.627 12 0 12C6.627 12 12 17.373 12 24C12 17.373 17.373 12 24 12C17.373 12 12 6.627 12 0Z"/>
    </svg>
   </div>
 ...
 listEl.innerHTML = aiChatMessages.map((m, idx) => `
  <div style="align-self:${m.role==='user'?'flex-end':'flex-start'};max-width:86%;display:flex;flex-direction:column;gap:4px;">
   <div style="background:${m.role==='user'?'linear-gradient(135deg,#f5b301,#e8b64c)':'#131724'};color:${m.role==='user'?'#111':'#f1f5f9'};border:${m.role==='user'?'none':'1px solid rgba(155,114,203,0.3)'};padding:12px 15px;border-radius:18px;${m.role==='user'?'border-bottom-right-radius:4px;':'border-bottom-left-radius:4px;'}font-size:13.5px;line-height:1.55;white-space:pre-wrap;box-shadow:${m.role==='user'?'0 4px 14px rgba(245,179,1,0.25)':'0 4px 16px rgba(0,0,0,0.3)'};">
    ${m.role==='ai'?'<div style="display:flex;align-items:center;gap:5px;font-size:11px;font-weight:700;color:#9B72CB;margin-bottom:6px;"><span style="font-size:13px;">✦</span> GEMINI</div>':''}
    ${m.text}
   </div>
   ${m.role==='ai' && m.text !== '💭...' ? `
    <div style="display:flex;align-items:center;gap:8px;padding:0 4px;">
     <button onclick="speakText(aiChatMessages[${idx}].text)" title="Sunno" style="background:transparent;border:none;color:#94a3b8;font-size:13px;cursor:pointer;display:flex;align-items:center;gap:4px;padding:2px 6px;border-radius:6px;">
      <i class="ti ti-volume"></i> <span style="font-size:11px;">Speak</span>
     </button>
    </div>`:''}
  </div>`).join('');
...
1. "KISSE PAISE LENA HAI / WHO OWES ME MONEY / RECEIVABLES" poochne par SIRF Section 1 ke REAL PEOPLE (jinme 'INSE LENA HAI' likha ho) ka naam aur amount batao.
2. KABHI BHI Section 2 ke Personal Expenses (Food, Auto, Fuel, Medicine, Self, Doctor, Office, Electricity, Rent, etc.) ya Investments (Mutual Fund, Groww, etc.) ko "lena hai" me mat batana! Wo user ke khud ke kharche hain. Agar user specific kharch ke baare me poochhe, to batao "Ye aapka personal kharch hai".
...
2) QUESTION — user balance, party info, analysis, recent entries, savings goals/targets poochh raha hai:
   ...
   - RULE FOR RECEIVABLES: "Kisse paise lena hai" / "Who owes me money" poochne par SIRF Section 1 me diye REAL PEOPLE (e.g. Adarsh, Sagar, etc. jinme 'INSE LENA HAI' likha ho) ka naam batao. KABHI BHI Food, Auto, Fuel, Medicine, Self, Doctor, Mutual Fund, Groww jaise personal kharche ya investments ko "lena hai" me mat batana! NEECHE diye REAL DATA ke base par chhota jawaab (2-4 lines).
...
function switchAivMode(mode){
 ...
 if (mode === 'entry') {
  ...
  if (titleEl) titleEl.innerHTML = '<span style="background:linear-gradient(135deg,#9B72CB,#4E82EE,#F4AF5F);-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-weight:800;">✦ Ask Balance · Gemini</span>';
