...
async function startAiChatVoiceCapture(){
 ...
 if (!window.__micPermissionOk) {
  const allowed = await requestAppPermission('mic');
  if (!allowed) {
   if (msgEl) msgEl.textContent = "Mic permission denied";
   return;
  }
  window.__micPermissionOk = true;
 }
...
${summary}`;
  ...
  const reply = await _geminiRawFetch(body);
  ...
  speakText(reply, () => {
   ...
   // CONTINUOUS MODE in Ask Balance: automatically listen again for next question!
   const modal = document.getElementById('aiChatModal');
   if (modal && !modal.classList.contains('hidden')) {
    setTimeout(() => {
     if (document.getElementById('aiChatModal') && !aiChatVoiceActive) {
...
//
// MODEL NOTE: Supports latest Gemini 3.5 & 2.5 models with intelligent multi-model fallbacks
const GEMINI_MODELS = [
 "gemini-3.5-flash",
...
async function _geminiRawFetch(body, signal){
 ...
 for (const model of GEMINI_MODELS) {
  try {
   ...
   lastErr = err;
   if (err && err.name === 'AbortError') break; // user cancelled — don't try next model
  }
 }
...
async function startAiMicRecording(){
 ...
 if (!window.__micPermissionOk) {
  ...
  if (!allowed) {
   ...
   setAivCaption("Microphone allow nahi hua.");
   return;
  }
  window.__micPermissionOk = true;
 }
