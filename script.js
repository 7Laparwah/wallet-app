/* ===== Inline script block 1 (originally at char offset 238915) ===== */
// Artwork already says "Welcome Back! / Enter your 4-digit PIN to continue",
// so the small status pill (#pinTitleText/#pinSubText) only needs to show
// up when the app is in a different PIN state (setup, confirm, reset).
(function(){
 var titleEl = document.getElementById('pinTitleText');
 var lockScreen = document.getElementById('pinLockScreen');
 if(!titleEl || !lockScreen) return;
 function syncPinPillVisibility(){
  var isDefault = titleEl.textContent.trim() === 'Enter PIN';
  lockScreen.classList.toggle('pinCustomState', !isDefault);
 }
 new MutationObserver(syncPinPillVisibility).observe(titleEl, {childList:true, characterData:true, subtree:true});
 syncPinPillVisibility();
})();

/* ===== Inline script block 2 (originally at char offset 3077136) ===== */
// ===== PAY NOW (send money via UPI) =====
let pendingUpiPayment = null; // holds the not-yet-saved entry data while UPI app is open
...
// ----- QR scan (via camera capture + jsQR decode) -----
// File-capture based (not live video) kyunki isse camera-permission handling
// simple rehta hai aur WebView/PWA dono me reliably kaam karta hai.
// NOTE: jsQR library CDN se load hoti hai, isliye QR scan ke liye ek baar
// internet chahiye hi (offline file:// testing me bhi, kyunki library khud
// local file nahi hai).
let jsQrLoaded = false;
let jsQrLoading = false;
function loadJsQr(cb, onFail){
 if (jsQrLoaded && window.jsQR) { cb(); return; }
 if (jsQrLoading) {
  // already loading — poll a moment for it to finish
  const check = setInterval(() => {
   if (jsQrLoaded && window.jsQR) { clearInterval(check); cb(); }
  }, 200);
 ...
 s.onload = () => { jsQrLoaded = true; jsQrLoading = false; cb(); };
 s.onerror = () => {
  jsQrLoading = false;
  if (onFail) onFail();
 };
...
// Preload as soon as the Pay modal opens, so the file-picker isn't waiting
// on a slow network right when the user is ready to scan.
const _origOpenPayNowModal = openPayNowModal;
openPayNowModal = function(){
 _origOpenPayNowModal();
 loadJsQr(() => {}, () => {});
};
...
function handleQrFileScan(evt){
 ...
 st.textContent = 'QR padha ja raha hai...';

 loadJsQr(() => {
  const img = new Image();
  img.onload = () => {
   // Downscale large camera photos (phone photos can be 3000-4000px wide,
   // which makes jsQR slow/unreliable). Cap the longer side at 1200px.
   const maxDim = 1200;
   ...
   if (code && code.data) {
    try {
     ...
     const parsed = new URL(raw.replace(/^upi:\/\/pay/i, 'https://dummy'));
     const pa = parsed.searchParams.get('pa');
     const pn = parsed.searchParams.get('pn');
     if (pa) {
      document.getElementById('payNowUpiId').value = pa;
      if (pn) document.getElementById('payNowName').value = pn;
      st.textContent = 'QR se UPI ID mil gayi ✓';
     } else {
      st.textContent = 'Ye UPI QR nahi lag raha (data: "' + raw.slice(0,40) + '"), ID manually daalein';
     }
    } catch(e) {
     st.textContent = 'QR padhne me error, ID manually daalein';
    }
   } else {
    st.textContent = 'QR detect nahi hua — QR ko frame me clearly, close-up aur bina glare ke dobara try karein';
   }
  };
  img.onerror = () => { st.textContent = 'Image load nahi hui, dobara try karein'; };
  img.src = URL.createObjectURL(file);
 }, () => {
  st.textContent = 'QR scanner load nahi ho paaya — internet chahiye (ek baar), ya UPI ID manually daalein';
 });
...
function triggerUpiPayment(){
 ...
 const upiIdPattern = /^[\w.\-]{2,}@[a-zA-Z]{2,}$/;
 if (!upiIdPattern.test(upiId)) {
  errEl.textContent = 'Valid UPI ID daaliye (jaise name@oksbi)';
 ...
 // IMPORTANT: entry abhi SAVE nahi ho rahi — sirf memory me hold ho rahi hai.
 // Save sirf tabhi hoga jab user "Successful" confirm karega. "Fail/Cancelled"
 // dabane par ye data discard ho jaayega, koi entry nahi banegi.
 pendingUpiPayment = {
  id: 'e_' + Date.now(),
  party: payeeName,
  method: 'UPI',
...
// App-specific deep links (gpay://, phonepe://, paytmmp://) launch that app
// DIRECTLY instead of going through Android's generic-intent chooser for
// `upi://`. GPay's fraud/security filter treats a generic-intent launch
// (especially from a browser/PWA context) with more suspicion than a
// direct app-scheme launch, so offering the specific app first can reduce
// "declined for security reasons" errors. Generic upi:// stays as a fallback
// for any UPI app not in this list.
function buildUpiParams(upiId, payeeName, amount, note){
 const txnRef = 'TXN' + Date.now();
 return `pa=${encodeURIComponent(upiId)}&pn=${encodeURIComponent(payeeName)}&am=${amount.toFixed(2)}&cu=INR&tn=${encodeURIComponent(note)}&tr=${txnRef}&tid=${txnRef}`;
}
...
function launchUpiApp(scheme){
 ...
 closeUpiAppChooser();
 const schemeMap = {
  gpay: 'tez://upi/pay?' + params,       // GPay's own scheme is "tez://"
  phonepe: 'phonepe://pay?' + params,
  paytm: 'paytmmp://pay?' + params,
  other: 'upi://pay?' + params           // generic — lets phone show its own chooser
 };
 window.location.href = schemeMap[scheme] || schemeMap.other;
}

// ----- Manual copy-paste fallback -----
// Ye kabhi decline nahi hota kyunki koi automatic UPI intent trigger nahi ho
// raha — user khud apne UPI app me manually details paste karke pay karta
// hai. Kyunki koi app-switch intent nahi hua, visibilitychange reliably
// fire nahi hoga, isliye yahan seedha confirm-prompt dikha dete hain.
async function copyUpiDetails(){
 ...
 try {
  ...
  // Clipboard API sometimes blocked (e.g. no HTTPS/permission) — show it inline instead.
  alert('Copy nahi ho paaya, manually likh lo:\n\n' + text);
 }
 closeUpiAppChooser();
 // Direct confirm-prompt, since there's no automatic redirect to wait on.
...
function hideGoogleLoginPage() {
  ...
  if (pinScreen && appContainer) {
    ...
    if (pinMode === 'create') {
      ...
      document.getElementById('pinTitleText').textContent = "Enter PIN";
      document.getElementById('pinSubText').textContent  = "Enter your 4-digit PIN";
...
function buildBalanceSummaryText(maxParties){
 ...
 // SPEED FIX: voice quick-add (compact mode) sirf top parties bhejta hai,
 // na ki har party — bade party-list wale users ke liye prompt chhota
 // rehta hai, isliye Gemini ka response fast aata hai. Chat/"Ask Balance"
 // (maxParties na diya ho) me poori list milti hai, quality kam nahi hoti.
 const rowsToShow = maxParties
  ? [...rows].sort((a,b)=>Math.abs(b.net)-Math.abs(a.net)).slice(0, maxParties)
  : rows;
 const lines = rowsToShow.map(r => {
  const status = r.net === 0 ? "settled/barabar" : (r.net > 0 ? `inse ₹${Math.abs(r.net).toLocaleString('en-IN')} lena hai` : `inko ₹${Math.abs(r.net).toLocaleString('en-IN')} dena hai`);
  return `- ${r.party}: total paid ₹${r.paid.toLocaleString('en-IN')}, total received ₹${r.received.toLocaleString('en-IN')} → ${status}`;
 });
 const omittedNote = (maxParties && rows.length > rowsToShow.length) ? `\n(+${rows.length - rowsToShow.length} aur parties, chhote balance wale)` : "";
 return `Overall: total paid ₹${totalPaid.toLocaleString('en-IN')}, total received ₹${totalReceived.toLocaleString('en-IN')}.\n\nParty-wise (top ${rowsToShow.length}):\n` + lines.join("\n") + omittedNote;
}

// ===== Full app context for "Ask Balance" voice/text questions =====
// Sirf party-wise balance kaafi nahi tha — user date-range (aaj/kal/is
// week/last month/...), payment-method-wise, search, analytics (top
// expenses, daily average), aur budget (Expense Limit goals) ke baare
// me bhi poochh sakta hai. Ye function sab real data (localStorage se,
// same source jo Wallet/Analytics/Goals screens use karte hain) ko ek
// Hinglish text me jod deta hai jo Gemini ko bheja jaata hai — jawaab
// sirf isi data ke base par milta hai.
// NOTE: entries me abhi "category" (Food/Travel/Petrol...) field nahi
// hai — sirf party/method/notes hai, isliye category-wise breakdown
// yahan include nahi kiya gaya hai.
function buildAiContextText(compact){
 ...
 const balanceSection = buildBalanceSummaryText(compact ? 15 : undefined);
...
`Aaj: expense ${toINR(todayR.exp)}, income ${toINR(todayR.inc)} (${todayR.count} entries)
Kal: expense ${toINR(yestR.exp)}, income ${toINR(yestR.inc)} (${yestR.count} entries)
...
Is saal (${curYear}): expense ${toINR(yearR.exp)}, income ${toINR(yearR.inc)}`;
 ...
 const paymentMethodSection = `All-time (payment/expense side): ${methodBreakup(entries)}\nIs mahine: ${methodBreakup(monthEntries)}`;
 ...
 // ---- 7) Search-friendly entries list (party/amount/date/method/notes keyword search ke liye) ----
 const searchCap = compact ? 25 : 60;
 ...
 // ---- 8) Goals (savings targets funded by manual contributions — same
 // `goals` array + fmtGoalMoney() data the Goals tab itself renders from,
 // so the AI always answers with numbers that exactly match what's on
 // screen). ----
 ...
 try {
  ...
  if (activeGoals.length > 0) {
   goalsSection = activeGoals.map(g => {
    ...
    const statusWord = g.status === 'completed' ? 'complete ho gaya' : `target date ${g.targetDate}`;
    return `- "${g.name}" goal: ${toINR(g.saved)} / ${toINR(g.target)} (${pct}%) saved, ${statusWord}`;
   }).join("\n");
  }
 } catch(err) { /* goals load fail ho to bhi baaki context mil jaaye */ }

 return `BALANCE (party-wise, all-time):\n${balanceSection}\n\nDATE-WISE TOTALS:\n${dateWiseSection}\n\n${analysisSection}\n${totalsSection}\n\nPAYMENT METHOD WISE:\n${paymentMethodSection}\n\nANALYTICS:\n${analyticsSection}\n\nENTRIES LIST (latest ${searchCap}, party/amount/date/method/notes se search karne ke liye):\n${recentSection}\n\nGOALS/BUDGET/TARGETS:\n${goalsSection}`;
}
...
async function sendAiChatMessage(){
 ...
 try {
  ...
  const prompt = `Tum ek personal finance app ("DayToDay") ke andar ek helpful assistant ho. User apne paise ke baare me kuch bhi poochh sakta hai — balance, is mahine ka analysis/kharch, recent entries, ya goals/targets ka progress. NEECHE diye gaye REAL DATA ke alawa kuch bhi mat maano ya banao — agar data me answer na ho to saaf bolo "ye jaankari mere paas nahi hai".
...
Chhota, seedha, Hinglish me jawaab do (2-4 lines se zyada nahi, jab tak zaroorat na ho). Rupee amounts ko ₹ ke saath likho.`;
...
function parseVoiceToQuickAddLine(said){
 ...
 let method = "Cash";
 if (/upi/.test(lower)) method = "UPI";
 else if (/bank|online/.test(lower)) method = "Bank";
 else if (/cash/.test(lower)) method = "Cash";
 ...
 let stripped = raw
  ...
  .replace(/\b(cash|upi|bank|online|ke|liye|diye|de|do|pay|paid|recd|received|mila|mile|kiye|kiya|kar|karo|hai|the|ka|ki|ko|se|aur|and|for)\b/gi, "")
  .trim();
 const partyGuess = stripped.split(/\s+/).filter(Boolean)[0] || "";
 const matchedParty = matchPartyName(partyGuess) || partyGuess;

 // Notes extraction: sirf DO trigger words — "se" aur "for". Jo bhi in
 // shabdon ke TURANT BAAD bacha hua poora sentence hai, wahi notes hai
 // (verb samet, chhota nahi karna). Koi aur trigger nahi hai — "ke liye"/
 // "liye" par trigger NAHI hota.
 let notes = "";
...
// Jab user live caption box me khud type/edit karta hai (cursor rakh ke
// text badalta hai), tab bhi wahi parsing dobara chalti hai jo voice final
// result par chalti thi — taaki edited text se hi amount/party/method sahi
// nikle aur Confirm & Save us par kaam kare.
...
// AI (Gemini, free tier) se text ko parse karta hai — amount/party/method/
// type nikalta hai, local regex-based parseVoiceToQuickAddLine() se zyada
// samajhdaari se. `auto=true` matlab ye mic band hone par khud-ba-khud
// chala hai (is case me chup-chaap fail ho sakta hai, koi bhi UI-blocking
// error nahi dikhega — jo already local-parse se ban chuka hai wahi
// dikhta rahega). `auto=false` matlab user ne khud button dabaya hai
// (jaise text edit karne ke baad dobara try karne ke liye).
async function aiImproveQuickAddText(auto){
 ...
 if (!apiKey) {
  ...
  return; // auto mode: chup-chaap skip, local parse pehle se dikh rahi hai
 }
 if (!text) {
 ...
 if (auto) statusEl.textContent = "✨ AI se entry samjhi ja rahi hai...";

 try {
  // BUGFIX: pehle ye hamesha maan leta tha ki text ek TRANSACTION hai —
  // agar koi sawaal type/edit kare (jaise "adarsh ka balance kya he?"),
  // to amount/party khaali/galat nikalta tha, aur Confirm dabane par
  // "Sahi amount likho" jaisa confusing error deta tha. Ab AI Mic (audio)
  // ki tarah yahan bhi pehle "intent" (transaction/question) decide hota
  // hai — sawaal ho to seedha jawaab milta hai, entry form ko haath nahi
  // lagaya jaata.
  ...
  const prompt = `Tum ek Hindi/Hinglish personal-finance app ke andar ek assistant ho. User ne ye likha: "${text}"

Pehle decide karo "intent" kya hai:
1) TRANSACTION — ek expense/income bata raha hai (jaise "200 adarsh ko cash diye"). Is case me "intent":"transaction" do, aur:
   - amount (number, sirf digits), party (naam), method ("Cash"/"UPI"/"Bank" — na bola ho to "Cash"), type ("expense" agar diya/pay kiya, "income" agar mila/receive hua), notes: sirf DO trigger words hain — "se" aur "for". Jo bhi in shabdon ke turant baad bacha hua poora text hai, wahi word-for-word notes me do (chhota mat karo). Dono me se koi na ho to notes "" do — koi aur trigger use mat karo.
   Existing party names (close match kare to WAHI naam exact spelling ke saath use karo): ${JSON.stringify(partyNames)}

2) QUESTION — balance, is mahine ka analysis/kharch, recent entries, ya goals/targets ke baare me sawaal poochh raha hai (jaise "adarsh ka balance kya hai", "is mahine kitna kharch hua", "mera savings goal kitna complete hua"). Is case me "intent":"question" do, aur:
   - answer: NEECHE diye REAL DATA ke base par chhota Hinglish jawaab (2-3 lines se zyada nahi). Data me na ho to saaf "ye jaankari mere paas nahi hai" bolo — kabhi kuch mat banao.

REAL DATA (balance, is-mahine ka analysis, recent entries, goals/targets):
${balanceSummary}

EXAMPLE 1 — "400 ritesh ko cash pay kiye advance me" -> {"intent":"transaction","amount":400,"party":"Ritesh","method":"Cash","type":"expense","notes":""}
EXAMPLE 1B ("se" trigger) — "200 rupees mahi ko upi se pay kiye dawai kharidne ke liye" -> {"intent":"transaction","amount":200,"party":"Mahi","method":"UPI","type":"expense","notes":"pay kiye dawai kharidne ke liye"}
EXAMPLE 1C ("for" trigger) — "200 rupees for medicine paid to mahi" -> {"intent":"transaction","amount":200,"party":"Mahi","method":"Cash","type":"expense","notes":"medicine paid to mahi"}
EXAMPLE 2 — "adarsh ka balance kya he?" -> {"intent":"question","answer":"abc se aapko ₹200 lene hain."}
EXAMPLE 3 — "is mahine kitna kharch hua?" -> {"intent":"question","answer":"Is mahine ab tak ₹4,500 kharch hua hai, sabse zyada Food me."}
EXAMPLE 4 — "savings goal kitna complete hua?" -> {"intent":"question","answer":"Aapka \\"Monthly Savings Goal\\" ₹6,000 / ₹15,000 (40%) complete hai."}`;

  const result = await callGeminiJSON(prompt, schema);
  if (!result) throw new Error("AI key set nahi hai");

  if (result.intent === "question") {
   // Ye ek sawaal tha, entry nahi — qaLine ko chhedte nahi taaki galti se
   // koi khaali/galat entry Confirm par save na ho jaaye.
  ...
  document.getElementById('qaLine').value = `${amount},${matchedParty},,${method},${cleanNotes}`;
  statusEl.textContent = `✨ AI ne samjha: ₹${amount || '?'} · ${matchedParty || '(party set nahi)'} · ${method}${cleanNotes ? ' · ' + cleanNotes : ''} — theek hai to Confirm dabao (ya text edit karo)`;
 } catch (err) {
  // AI fail ho gaya — jo local (regex) parsing pehle se ho chuki thi wahi
  // qaLine me hai, entry banna nahi rukega. Bas ek chhota sa notice.
  onQaLiveCaptionEdit();
...
function speakText(text, onEnd){
 try {
  ...
  // Safety fallback: agar kisi wajah se onend/onerror kabhi na chale
  // (kuch browsers me rare bug hota hai), to bhi agla step atka na rahe.
...
async function blobToWavBase64(blob){
 ...
 // SPEED FIX: 16kHz par resample karte hain. Recording aksar 44.1kHz ya
 // 48kHz hoti hai — insaan ki awaaz/speech ke liye itni high quality ki
 // zaroorat nahi (Gemini khud bhi audio ko ~16kHz par hi process karta
 // hai), lekin WAV uncompressed hone ki wajah se file size seedha sample
 // rate ke proportion me badhta hai. 16kHz par resample karne se file
 // ~3x chhoti ho jaati hai → upload aur AI processing dono kaafi fast.
 const TARGET_RATE = 16000;
 let finalBuffer = audioBuffer;
 if (audioBuffer.sampleRate !== TARGET_RATE) {
  try {
   const OfflineCtxClass = window.OfflineAudioContext || window.webkitOfflineAudioContext;
   const offlineCtx = new OfflineCtxClass(1, Math.ceil(audioBuffer.duration * TARGET_RATE), TARGET_RATE);
   ...
   // Resample fail ho jaaye (rare) to original quality me hi bhej do —
   // thodi badi file jaayegi, lekin rukna nahi chahiye.
   finalBuffer = audioBuffer;
  }
 }
 const wavBlob = _encodeWavFromAudioBuffer(finalBuffer);
 return await blobToBase64(wavBlob);
}
function _encodeWavFromAudioBuffer(audioBuffer){
 // Mono me downmix karte hain (Gemini ko stereo ki zaroorat nahi, aur
 // file size bhi chhota rehta hai — upload jaldi hota hai).
...
// AI kabhi-kabhi transcript me "UPI"/"Bank" saaf sunta hai (transcript
// field me sahi likha aata hai) lekin apna "method" field phir bhi galti
// se "Cash" bhej deta hai — do alag fields hain, ek dusre se consistent
// hona guaranteed nahi hai. Isliye method ko AI ke field par chhodne ke
// bajaye transcript ke asli shabdon se dobara (deterministically) verify
// karte hain — jo bhi transcript me clearly bola gaya ho WAHI final
// maana jaata hai, AI ke apne method field se zyada bharosemand.
function detectMethodFromTranscript(transcript){
 const t = (transcript || "").toLowerCase();
 if (/\b(upi|gpay|g-pay|google\s*pay|phonepe|phone\s*pe|paytm)\b/.test(t)) return "UPI";
 if (/\b(bank|neft|imps|rtgs|transfer|account\s*se|khaate\s*se)\b/.test(t)) return "Bank";
 if (/\b(cash|nakad|nakd)\b/.test(t)) return "Cash";
 return null; // transcript me method ka koi zikr hi nahi — AI ke field par bharosa karo
}

async function aiMicToEntry(base64Audio, mimeType){
 const sel = document.getElementById('partySel');
 const partyNames = sel ? Array.from(sel.options).map(o => o.value).filter(Boolean) : [];
 const balanceSummary = buildAiContextText(true); // compact=true → tez response (dekho buildAiContextText comment)
 const schema = {
  type: "OBJECT",
  properties: {
   intent: { type: "STRING", enum: ["transaction", "question", "action"] },
 ...
 const promptText = `Tum ek Hindi/Hinglish personal-finance app ke andar ek voice assistant ho. Attached audio clip suno — user Hindi/Hinglish me bol raha hai.

User teen tarah ki cheezein bol sakta hai — pehle decide karo "intent" kya hai:

1) TRANSACTION — user ek NAYI expense/income bata raha hai (jaise "200 adarsh ko cash pay kiye advance me"). Is case me "intent":"transaction" do, aur:
   - transcript: jo bola gaya uska Roman/Hinglish likha hua poora text
   - amount: number, sirf digits
   - party: jisko/jisse paisa gaya/aaya uska naam
   - method: audio me DHYAN SE suno — agar "UPI", "GPay", "PhonePe", "Paytm" jaisa kuch bola gaya ho to "UPI" do; agar "bank", "transfer", "neft", "account se" jaisa kuch bola gaya ho to "Bank" do; agar "cash"/"nakad" bola ho YA method bilkul na bola gaya ho tabhi "Cash" do. GALTI se hamesha "Cash" mat de do — pehle poora audio dhyan se sun kar check karo ki UPI/Bank ka koi mention hai kya, tabhi decide karo.
   - type: "expense" agar paisa diya/pay kiya/kharch kiya, "income" agar paisa mila/receive hua/aaya
   - notes: sirf DO trigger words hain — "se" aur "for". Jo bhi in shabdon ke TURANT BAAD bacha hua POORA sentence hai — wahi word-for-word (verb/reason sab samet) notes me daalo, usse chhota mat karo. Example: "upi se pay kiye dawai kharidne ke liye" -> "se" ke baad = "pay kiye dawai kharidne ke liye" — poora yahi notes hai. Example: "200 for medicine paid to mahi" -> "for" ke baad = "medicine paid to mahi". Dono me se koi trigger na ho to notes "" do — koi aur trigger (jaise "ke liye") use mat karo.

2) QUESTION — user balance, date-wise kharch (aaj/kal/is week/last week/is mahina/last mahina/is saal), payment-method-wise (UPI/Cash/Bank), search (kisi party/keyword/amount se), analytics (top expenses, daily average, weekly trend), budget/goal status, ya money-saving tips/insights maang raha hai. Is case me "intent":"question" do, aur:
   - transcript: jo sawaal poocha gaya
   - answer: SEEDHA aur SIRF asli jawaab do — koi bhi filler/preamble jaisa "theek hai", "main samajh gaya", "chaliye batata hoon", "achha sawaal hai" KABHI mat likho. Pehle hi shabd se seedha fact/number/answer shuru karo. NEECHE diye REAL DATA ke base par chhota jawaab (2-4 lines se zyada nahi). Tips/insights wale sawaal me REAL DATA dekh kar khud reasoning se practical Hinglish tips do (jaise sabse zyada kis party/method pe kharch ho raha hai, wahan kam karne ka suggestion). Data me kuch specific na mile to saaf bolo "ye jaankari mere paas nahi hai" — kabhi bhi number/fact khud se mat banao.
   - IMPORTANT: entries me "category" (Food/Travel/Petrol jaisa) field STORE nahi hoti — sirf party/method/notes hoti hai. Agar user kisi category ke baare me poochhe (jaise "Food par kitna kharch hua"), to REAL DATA ke party naam/notes me us keyword se milta-julta match dhoondo aur bolo ki ye party/notes ke naam ke base par estimate hai, exact category tracking abhi available nahi hai.
   - Agar user koi INSTRUCTION/order de (jaise "aage se bar bar 'theek hai' mat bolo", "seedha jawaab do", "chhota jawaab do"), to us instruction ko turant maano aur usi turn se follow karna shuru karo — us par bhi koi filler/acknowledgement mat bolo, bas jo maanga gaya wo seedha karke dikhao.

3) ACTION — user apni AAKHRI (sabse recent) entry ko EDIT ya DELETE karne ko bol raha hai (jaise "last transaction delete karo", "pichli entry hata do", "last expense ka amount 500 kar do", "income update karo 2000 kar do"). Is case me "intent":"action" do, aur:
   - actionType: "delete_last_entry" (delete/hatao/remove bola) YA "update_last_entry" (amount/party/notes/type badalne ko bola)
   - update_last_entry ke liye jo bhi field badalni bataayi gayi ho wahi bhejo (amount/party/method/type/notes) — baaki khaali chhodo, wo purani value se hi rahegi
   - answer: user ko confirm karne ke liye chhota Hinglish message (jaise "Aakhri entry delete kar di gayi." ya "Amount ₹500 update kar diya.")
   NOTE: sirf tabhi "action" do jab user EXPLICITLY edit/update/delete/remove/hatao/badlo bole kisi PURANI entry ke baare me — agar shaq ho ki naya transaction hai ya sawaal hai, to wahi maano (action sabse aakhri option hai).

Existing party names (transaction match karne ke liye — agar close match kare to WAHI naam exact spelling ke saath use karo): ${JSON.stringify(partyNames)}

REAL DATA (balance, date-wise totals, payment-method wise, analytics, entries list, goals/budget):
${balanceSummary}

EXAMPLE 1 (transaction, no trigger word — notes empty) — "400 ritesh ko cash pay kiye advance me" ->
{"intent":"transaction","transcript":"400 ritesh ko cash pay kiye advance me","amount":400,"party":"Ritesh","method":"Cash","type":"expense","notes":""}

EXAMPLE 1B (transaction, "se" trigger — POORA remainder notes me, chhota mat karo) — "200 rupees mahi ko upi se pay kiye dawai kharidne ke liye" ->
{"intent":"transaction","transcript":"200 rupees mahi ko upi se pay kiye dawai kharidne ke liye","amount":200,"party":"Mahi","method":"UPI","type":"expense","notes":"pay kiye dawai kharidne ke liye"}
(GALAT hota agar notes sirf "dawai kharidne" ya "dawai" itna hi hota — "se" ke baad ka POORA sentence chahiye, verb "pay kiye" bhi saath me.)

EXAMPLE 1C (transaction, method UPI dhyan se pakadna, "se" ke baad chhota text) — "500 rahul ko UPI se bheje" ->
{"intent":"transaction","transcript":"500 rahul ko UPI se bheje","amount":500,"party":"Rahul","method":"UPI","type":"expense","notes":"bheje"}
(GALAT hota agar "UPI se bheje" sunne ke baad bhi method "Cash" de diya jaata — audio me method ka mention hamesha dhyan se sunna hai.)

EXAMPLE 1E (transaction, "for" trigger) — "200 rupees for medicine paid to mahi" ->
{"intent":"transaction","transcript":"200 rupees for medicine paid to mahi","amount":200,"party":"Mahi","method":"Cash","type":"expense","notes":"medicine paid to mahi"}

EXAMPLE 1D (transaction, method Bank) — "2000 salary bank me aaye" ->
{"intent":"transaction","transcript":"2000 salary bank me aaye","amount":2000,"party":"Salary","method":"Bank","type":"income","notes":""}

EXAMPLE 2 (question - balance) — "adarsh ka balance kitna hai" ->
{"intent":"question","transcript":"adarsh ka balance kitna hai","answer":"abc se aapko ₹200 lene hain."}

EXAMPLE 3 (question - date-wise) — "aaj kitna kharch hua" ->
{"intent":"question","transcript":"aaj kitna kharch hua","answer":"Aaj ab tak ₹450 kharch hua hai, 2 entries me."}

EXAMPLE 4 (question - payment method) — "UPI se kitna payment hua" ->
{"intent":"question","transcript":"UPI se kitna payment hua","answer":"UPI se ab tak total ₹8,200 payment hua hai."}

EXAMPLE 5 (question - search) — "amazon ke transactions dikhao" ->
{"intent":"question","transcript":"amazon ke transactions dikhao","answer":"Amazon ki 2 entries mili: 12-Jul ko ₹1,200 aur 20-Jul ko ₹650."}

EXAMPLE 6 (question - analytics) — "sabse zyada paisa kis party pe gaya" ->
{"intent":"question","transcript":"sabse zyada paisa kis party pe gaya","answer":"Is mahine sabse zyada Rahul pe ₹3,500 gaya hai."}

EXAMPLE 7 (question - budget) — "budget kitna bacha hai" ->
{"intent":"question","transcript":"budget kitna bacha hai","answer":"Monthly Expense Limit ₹15,000 / ₹20,000 (75%) use ho chuka hai, ₹5,000 bacha hai."}

EXAMPLE 8 (question - tips) — "paise bachane ki tips do" ->
{"intent":"question","transcript":"paise bachane ki tips do","answer":"Is mahine sabse zyada kharch Rahul aur UPI payments me ho raha hai — wahan thoda control karo to savings badh sakti hai."}

EXAMPLE 9 (action - delete) — "last transaction delete karo" ->
{"intent":"action","transcript":"last transaction delete karo","actionType":"delete_last_entry","answer":"Aakhri entry delete kar di gayi."}

EXAMPLE 10 (action - update) — "last entry ka amount 500 kar do" ->
{"intent":"action","transcript":"last entry ka amount 500 kar do","actionType":"update_last_entry","amount":500,"answer":"Aakhri entry ka amount ₹500 update kar diya."}`;
...
async function toggleAiMic(){
 ...
 // AI abhi jawaab bol raha hai — tap karke turant cancel karo (jaise
 // conversation ko rok dena), taaki user chahe to turant naya tap karke
 // fresh se bol sake.
 ...
 // SINGLE-TAP SYSTEM: agar already record ho raha hai to ye sirf ek manual
 // "jaldi rok do" backup hai — normally VAD (neeche) khud hi bolna khatam
 // hote hi rok deta hai, dobara tap karne ki zaroorat nahi hoti.
 if (aiMicActive && aiMicRecorder && aiMicRecorder.state === 'recording') {
  aiMicRecorder.stop();
  return;
 }

 aiMicKeepListening = false; // fresh tap — naya conversation shuru
 await startAiMicRecording();
}

// Ek hi tap se mic on karta hai, recording start karta hai, aur voice
// activity detection (VAD) laga deta hai jo khud bolna khatam hone par
// (thodi si chup rehne ke baad) recording rok kar process shuru kar deta
// hai — isliye dobara tap karne ki zaroorat nahi padti.
async function startAiMicRecording(){
 ...
 try {
  ...
  // Voice Activity Detection: bolna shuru hote hi note karo, phir jab
  // ~1.2 second chup ho jaaye to recording khud stop kar do. Agar kisi
  // wajah se VAD set up na ho paaye (purana browser), to bhi manual tap
  // (upar toggleAiMic me) backup ke taur pe kaam karta rehta hai.
  ...
  aiMicRecorder.onstop = async () => {
   ...
   if (aiMicChunks.length === 0) {
    setAiOrbState('idle');
    setAivState('idle');
    setAivCaption("Kuch record nahi hua — mic dabao aur bolo.");
    if (stateTextEl) { stateTextEl.textContent = "AI Mic is off — tap to speak."; stateTextEl.style.color = "#888"; }
    statusEl.textContent = "Kuch record nahi hua — mic dabao aur bolo.";
    return;
   }
   ...
   try {
    ...
    try {
     // Primary path: convert to WAV — reliably supported by Gemini on
     // every browser/device, fixes the "text nahi dikhta" / inconsistent
     // result bug that webm/mp4 caused.
     base64Audio = await blobToWavBase64(blob);
     mimeForApi = 'audio/wav';
    } catch (convErr) {
     // Bahut hi rare fallback — agar kisi wajah se WAV conversion hi fail
     // ho jaaye (jaise device decodeAudioData support na kare), to purana
     // raw-blob tareeka try karo taaki bilkul kaam karna band na ho.
    ...
    // Agar isi beech user ne "X" dabaakar screen band kar di ho (Gemini
    // ka jawaab aane tak wait karte hue), to yahin ruk jao — na entry save
    // karo, na kuch bolo. Isse pehle "close karne ke baad bhi purana
    // jawaab bolta rehta tha" wala bug fix hota hai.
    ...
    if (result.intent === "question") {
     // Ye ek balance/party SAWAAL tha, entry nahi — isi mic se jawaab bhi
     // mil jaata hai, alag Chat kholne ki zaroorat nahi. Entry form ko
     // chhedte nahi (qaLine khaali hi rehta hai) taaki galti se koi entry
     // save na ho jaaye.
     aiMicKeepListening = true; // jawaab ke baad mic khud dobara sunna shuru karega — agla sawaal poochne ke liye tap nahi karna padega
     const askedText = result.transcript || "";
     const answer = (result.answer || "Sorry, couldn't find an answer.").trim();
     ...
     speakText(answer, () => {
      ...
      if (stateTextEl) { stateTextEl.textContent = "🔴 Sun raha hoon..."; stateTextEl.style.color = "#a855f7"; }
      // Tap ki zaroorat nahi — mic khud dobara on ho jaata hai taaki
      // conversation chalti rahe. Agar cancel karna ho to mic dabao.
      if (aiMicKeepListening && isAiVoiceScreenOpen()) startAiMicRecording();
     });
     btn.disabled = false;
     return; // transaction wala flow neeche skip
    }

    if (result.intent === "action") {
     // "Last transaction delete/edit karo" jaisa voice command — koi naya
     // entry save nahi hota, existing sabse recent entry par action hota hai.
     aiMicKeepListening = true;
     const askedText = result.transcript || "";
     let answer;
     try {
    ...
    const rawPartyMatch = matchPartyName(result.party); // null agar list me confident match na mila ho
    const matchedParty = rawPartyMatch || result.party || "";
    const amount = Number(result.amount) || "";
    const transcript = result.transcript || "";
    // Transcript ke actual shabdon se method verify karo — agar wahan
    // clearly UPI/Bank/Cash bola gaya hai to wahi use hoga, chahe AI ke
    // apne "method" field me kuch aur (galti se) aaya ho.
    const method = detectMethodFromTranscript(transcript) || normalizeVoiceMethod(result.method);
    if (result.type === "income" || result.type === "expense") quickAddVoiceType = result.type;
    const cleanNotes = sanitizeVoiceNotes(result.notes, transcript); // sirf "se"/"for" trigger ke baad ka poora text — AI ka diya hua as-is use hota hai, koi truncate nahi

    captionEl.value = transcript;
    // Notes field me clean/sanitized note use hota hai — agar Gemini ne
    // reason diya tha to wahi (ya uska chhota hissa) save hota hai; kuch
    // reason bola hi nahi gaya tha to notes khaali rehta hai (bug nahi).
    document.getElementById('qaLine').value = `${amount},${matchedParty},,${method},${cleanNotes}`;
    ...
    if (amount && rawPartyMatch) {
     // Sab kuch clear/confident hai — Gemini Live jaisa: samjho, bolo,
     // aur khud save kar do (extra Confirm tap ki zaroorat nahi). Ek hi
     // tap me poora kaam ho jaata hai: suno → samjho → dikhao → save karo.
     const summaryLine = `₹${amount} · ${matchedParty} · ${method}${cleanNotes ? ' · ' + cleanNotes : ''}`;
     statusEl.textContent = `✨ AI ne samjha: ${summaryLine} — Saving...`;
     setAiOrbState('speaking');
     setAivState('speaking');
     setAivCaption(summaryLine + " — saving...");
     if (stateTextEl) { stateTextEl.textContent = "🔊 AI bol raha hai..."; stateTextEl.style.color = "#a855f7"; }
     const speakLine = `Maine samjha: ${amount} rupaye, ${matchedParty} ke saath, ${method} se ${typeWord}.${noteSpeak} Entry save kar raha hoon.`;
     speakText(speakLine, () => {
      submitQuickAdd();
      setAiOrbState('idle');
      setAivState('idle');
     ...
     // Kuch confirm nahi ho paya (amount ya party clear nahi) — safety ke
     // liye khud se save nahi karta, sirf bol/bata ke user se manual
     // Confirm maangta hai.
     const summaryLine = `₹${amount || '?'} · ${matchedParty || '(party set nahi)'} · ${method}${cleanNotes ? ' · ' + cleanNotes : ''}`;
     statusEl.textContent = `✨ AI ne samjha: ${summaryLine} — theek hai to Confirm dabao (ya text edit karo)`;
     setAiOrbState('speaking');
     setAivState('speaking');
     setAivCaption("Puri tarah samajh nahi paya — " + summaryLine + ". Check karke Confirm karo.");
     if (stateTextEl) { stateTextEl.textContent = "🔊 AI bol raha hai..."; stateTextEl.style.color = "#a855f7"; }
     const speakLine = `Maine puri tarah nahi samjha. ${amount ? ('₹' + amount) : 'amount'} aur party check karke Confirm dabao.`;
     speakText(speakLine, () => {
      setAiOrbState('idle');
      setAivState('idle');
    ...
    statusEl.textContent = "⚠️ AI se nahi ho paya: " + err.message + " — chaho to upar backup mic (🎙️ chhota icon) try karo.";
...
// VAD: audio volume ko monitor karta hai. Jaise hi speech shuru hoke
// ~1.2 second ke liye chup ho jaati hai, recording khud stop kar deta hai
// — isi wajah se dobara tap karne ki zaroorat khatam ho jaati hai. Ek
// safety max-duration bhi hai taaki background noise ki wajah se mic
// hamesha ke liye on na reh jaaye.
function setupAiMicAutoStop(stream, recorder){
 try {
  const AudioCtx = window.AudioContext || window.webkitAudioContext;
  if (!AudioCtx) return; // support na ho to manual tap-to-stop fallback rahega
  ...
  const SPEECH_THRESHOLD = 10;      // is se upar volume = user bol raha hai
  const SILENCE_MS_TO_STOP = 1200;  // bolna khatam hone ke baad itni der chup rehne par auto-stop
  const MAX_RECORD_MS = 30000;      // safety cap — hamesha ke liye on na rahe
  let hasSpoken = false;
  ...
  // VAD fail ho jaaye to bhi manual tap-to-stop backup existing hi hai
 }
}
...
function pinSubmit(){
 ...
 document.getElementById('pinErrorText').textContent = "PIN didn't match, please try again";
 pinFirstEntry = "";
 pinEntered = "";
 pinUpdateDots();
 document.getElementById('pinTitleText').textContent = "Set a PIN";
 ...
 document.getElementById('pinErrorText').textContent = "Wrong PIN, please try again";
 pinEntered = "";
 pinUpdateDots();
 }
 }
