...
//
// MODEL NOTE: Uses active Google Gemini REST API endpoints with intelligent fallback
const GEMINI_MODELS = [
 "gemini-1.5-flash",
 ...
 for (const model of GEMINI_MODELS) {
  ...
  try {
   const resp = await fetch(
    ...
    { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(body), signal: timeoutCtrl.signal }
   );
   clearTimeout(timer);
   ...
   if (!resp.ok) {
    ...
    console.warn(`[Gemini API] Model ${model} failed (${resp.status}): ${msg}. Trying fallback...`);
    continue;
   }
   ...
   if (err && err.name === 'AbortError') {
    if (signal && signal.aborted) break;
    console.warn(`[Gemini API] Model ${model} timed out. Trying fallback...`);
   }
  }
 }
...
function saveAiSettingsKey(){
 ...
 if (!val) { msgEl.textContent = "Please paste your API key."; msgEl.style.color = "#f06464"; return; }
 setGeminiKey(val);
 msgEl.textContent = "✅ Saved! AI features are active.";
