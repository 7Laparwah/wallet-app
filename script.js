...
User ka sawaal: "${question}"`;
  ...
  aiChatMessages[thinkingIdx] = { role: 'ai', text: "⚠️ AI error: " + err.message };
  renderAiChatMessages();
 } finally {
...
function getGeminiKey(){
 ...
 if (stored) {
  const val = _aiKeyDeobfuscate(stored).trim();
  if (val) return val;
 }
 if (typeof window !== 'undefined' && window.APP_CONFIG && window.APP_CONFIG.geminiApiKey) {
  return String(window.APP_CONFIG.geminiApiKey).trim();
 }
 return '';
}
function setGeminiKey(k){
 if (k && k.trim()) {
  localStorage.setItem('geminiApiKeyEnc', _aiKeyObfuscate(k.trim()));
 } else {
...
//
// MODEL NOTE: Active Google Gemini models with auto-discovery and intelligent fallback
...
async function getAvailableGeminiModels(apiKey, forceRefresh = false){
 ...
 try {
  ...
  const listTimer = setTimeout(() => listCtrl.abort(), 5000);
  ...
  if (resp.ok) {
   ...
   if (Array.isArray(data?.models)) {
    ...
    if (supported.length > 0) {
     // Rank flash models first (fastest for voice/real-time), newer versions first
     const flash = supported.filter(m => m.includes('flash') && !m.includes('lite')).sort((a, b) => b.localeCompare(a));
     const flashLite = supported.filter(m => m.includes('flash') && m.includes('lite')).sort((a, b) => b.localeCompare(a));
     const pro = supported.filter(m => m.includes('pro')).sort((a, b) => b.localeCompare(a));
     const rest = supported.filter(m => !flash.includes(m) && !flashLite.includes(m) && !pro.includes(m));

     _cachedGeminiModels = Array.from(new Set([...flash, ...flashLite, ...pro, ...rest]));
     console.log("[Gemini API] Dynamically discovered active models:", _cachedGeminiModels);
     return _cachedGeminiModels;
    }
   }
  } else if (resp.status === 400 || resp.status === 401 || resp.status === 403) {
   let msg = "Invalid/Expired API Key";
   try { const d = await resp.json(); if (d?.error?.message) msg = d.error.message; } catch(e){}
   throw new Error("Invalid/Expired API Key. Free key yahan se lein: aistudio.google.com/apikey (" + msg + ")");
  }
 } catch (e) {
  if (e && e.message && e.message.includes("Invalid/Expired API Key")) throw e;
  ...
  for (const apiVer of apiVersions) {
   ...
   try {
    ...
    if (!resp.ok) {
     ...
     // Check if error is due to Invalid/Expired API Key or Unregistered/Blocked caller
     if (resp.status === 400 || resp.status === 403 || resp.status === 401 || /api\s*key|caller|invalid|blocked|permission|quota/i.test(msg)) {
      throw new Error("Invalid/Expired API Key. Free key yahan se lein: aistudio.google.com/apikey (" + msg + ")");
     }
    ...
    if (signal) signal.removeEventListener('abort', onParentAbort);
    lastErr = err;
    if (err && err.message && (err.message.includes("Invalid/Expired API Key") || err.message.includes("API Key set nahi hai"))) {
     throw err; // Stop trying other models when key is invalid
    }
  ...
  <div class="modal" style="max-width:380px;">
   ...
   <div class="modal-body">
    ...
    <div style="position:relative;margin-bottom:10px;">
     <input id="aiKeyInput" type="password" placeholder="Paste AI API key here" value="" autocomplete="off"
...
async function saveAiSettingsKey(){
 ...
 try {
  ...
  setGeminiKey(val);
  const activeModel = models[0] || "Gemini Flash";
  if (msgEl) {
   msgEl.textContent = `✅ Connected! Model active: ${activeModel}`;
  ...
  if (msgEl) {
   msgEl.textContent = "❌ " + (err.message || "Invalid API key. Please check your key from aistudio.google.com/apikey");
...
async function stopAiMicRecording(hasSpokenManual){
 ...
 try {
  ...
  const qaLine = document.getElementById('qaLine');
  if (qaLine) qaLine.value = `${amount},${matchedParty},${entryDate},${method},${cleanNotes}`;

  const typeWord = (quickAddVoiceType === "income") ? "received" : "paid";
  const noteSpeak = cleanNotes ? ` Note: ${cleanNotes}.` : "";

  if (amount && (rawPartyMatch || matchedParty)) {
   ...
   const speakLine = `₹${amount}, ${matchedParty}, ${method}, ${typeWord}.${noteSpeak} Saving entry.`;
   speakText(speakLine, () => {
    submitQuickAdd();
    setAiOrbState('idle');
    setAivState('idle');
  ...
  setAiOrbState('idle');
  setAivState('idle');
  const errText = "⚠️ AI processing failed: " + (err.message || err);
  setAivCaption(errText);
  if (statusEl) statusEl.textContent = errText;
  if (err && err.message && (err.message.includes("API Key") || err.message.includes("key"))) {
   setTimeout(() => { openAiSettingsModal(); }, 600);
   speakText("API Key missing or invalid. Please update AI key.");
  } else {
...
async function submitQuickAdd(){
 ...
 const party = matchPartyName(typedParty) || typedParty;
